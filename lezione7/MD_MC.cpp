...
//tail corrections and Evaluate properties of the initial configuration
vtail = (8*pi*npart*rho)*((1.0 / (9*pow(rcut, 9))) - (1.0 / (3*pow(rcut, 3))));
ptail = (32*pi*rho*npart)*((1.0 / (3*pow(rcut, 9))) - (1.0 / (2*pow(rcut, 3))));
Measure();
...
// distance i-j in pbc
      ...
      dr = dx*dx + dy*dy + dz*dz;
      dr = sqrt(dr);
      int bin = floor((dr*2 /(double) box)*100);
      double delta_r = (double) box / 200;
      gdir[bin] = gdir[bin] + ((double)2/(pow(dr+delta_r,3)-pow(dr,3)));

      if(dr < rcut)
      {
        vij = 1.0/pow(dr,12) - 1.0/pow(dr,6);
        v += vij;
        vij2 = 48*(1.0/pow(dr,12) - 0.5/pow(dr,6));
        P += vij2;
      }
    }          
  }
...
void Accumulate(void) //Update block averages
{
   ...
   for(int i = 0; i < 100; ++i)
   {
    blk_gdir_av[i] = blk_gdir_av[i] + gdir[i];
    gdir[i] = 0;
   }
...
void Averages(int iblk) //Print results for current block
{
  ...
  //g(r)
  for(int i = 0; i < 100; i++){
    stima_gdir = (3*blk_gdir_av[i])/(double) (blk_norm*rho*npart*4*pi);
    glob_gdir_av[i] += stima_gdir;
    glob_gdir_av2[i] += stima_gdir*stima_gdir;
    err_gdir[i]=Error(glob_gdir_av[i],glob_gdir_av2[i],iblk);
  }
...
if(iblk==nblk){
  for(int i = 0; i < 100; ++i){
    Gdir << setw(wd) << iblk << setw(wd) << (i+0.5)*((double)box/200) << setw(wd) << glob_gdir_av[i]/(double)iblk << setw(wd) << err_gdir[i] << endl;
  }
}
