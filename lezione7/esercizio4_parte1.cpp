...
//questo programma fa i grafici delle quantità T, P, U/N, K/N, E/N e g(r)
//usando i dati dei file creati dal programma NVE_NVT.exe.

int main(){
    TApplication app("app", 0, 0);
    ifstream Epot, Ekin, Etot, Temp, Pres, Gdir;
    unsigned int N = 20;
    double media = 0;
    double stddev = 0;
    double a = 0;
    ...
    for(int i=0; i < 100; i++){
        Gdir >> a >> a >> media >> stddev;
        graph6->SetPoint(i, a, media);
        graph6->SetPointError(i, 0.0, stddev);
    }

    TCanvas radial_distribution("g(r)", "g(r)");

    graph6->SetTitle("g(r)");
    graph6->GetXaxis()->SetTitle("r");
    graph6->GetYaxis()->SetTitle("g(r)");
    graph6->Draw("ALP");
