...
DNA(const vector<double>& x, const vector<double>& y, const vector<double> sequenza);
DNA(const DNA& copia);
...
//costruttore
DNA::DNA(const vector<double>& x, const vector<double>& y, const vector<double> sequenza){
    X = &x;
    Y = &y;
    geni.resize(x.size());
...
string DNA::to_stringa() const{
    string str = "{";
    for (size_t i = 0; i < geni.size()-1; i++){
        str += to_string(geni[i]) + ", ";
    }
    str += to_string(geni[geni.size()-1]);
    return str + "}";
}
...
//funzione che ottimizza il DNA scambiando sequenza di geni se tale scambio
